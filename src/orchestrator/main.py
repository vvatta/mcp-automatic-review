...
class MCPSandboxOrchestrator:
    ...
    async def _setup_isolation(self) -> Optional[docker.models.containers.Container]:
        """Setup Phase 2: Isolation Engine."""
        ...
        try:
            ...
            try:
                ...
                console.print(
                    "[yellow]Note: Use 'docker-compose build' to build the image[/yellow]"
                )
                return None

            # Prepare volume mounts - mount the workspace into the container
            volumes = {
    ...
    async def _run_interrogation(self) -> list[FuzzResult]:
        ...
        interrogator = MCPInterrogator(self.mcp_server_url, self.anthropic_api_key)
    ...
    def _generate_recommendations(self, risk_score: float) -> list[str]:
        ...
        if self.scan_result and self.scan_result.critical_count > 0:
            recommendations.append(
                f"🐛 Fix {self.scan_result.critical_count} critical vulnerabilities"
            )
